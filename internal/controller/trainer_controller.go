...
// Mutate verbs on cluster RBAC objects are limited to names this operator manages.
...
func (m *trainerActions) renderManifests(ctx context.Context, rr *types.ReconciliationRequest) error {
	...
	if clusterType == cluster.ClusterTypeOpenShift {
		rendered, err := m.renderResourceSet(resourceSet{
			name:         "manifests-openshift-rbac",
			subDir:       "manifests-openshift-rbac",
			templatePath: m.manifestsPath,
			namespace:    namespace,
			overlay:      openshiftOperandRBACOverlay,
		})
		if err != nil {
			return err
		}
		log.Info("Rendered manifests-openshift-rbac", "count", len(rendered))
		rr.Resources = append(rr.Resources, rendered...)
	}
